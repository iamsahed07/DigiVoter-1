        ...
        {!selectedElection && (
          <div className="grid grid-cols-1 gap-8 p-6">
            ...
            {upcomingElections.map(election => (
              ...
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-semibold">{election.name}</h3>
                <button className="text-sm bg-blue-500 text-white rounded-full px-4 py-2 hover:bg-blue-600 transition duration-300 ease-in-out" onClick={() => handleElectionClick(election.id)}>View Candidates</button>
              </div>
            </div>
          ))}
        </div>
